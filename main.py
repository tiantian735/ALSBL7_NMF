...
from NMF.nimfaRun import nimfaRun
from NMF.scikitRun import scikitRun
...
class MainWindow(wx.Frame):
    ...
    def OnUSM(self,event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            try:
                MainWindow.dataNew=USMSetup.usm(MainWindow.dataNew,dlg.USM_radius,dlg.USM_precent,dlg.USM_threhold)
...
class USMSetup(wx.Dialog):
    def __init__(self, parent):
        ...
        self.threholdtitle = wx.StaticText(self.panel, label="Threshold: ", pos=(20,100))
...
#Unsharp mask        
#class usm():
    def usm(data,radius,percent,threshold):
...
class nimfapanel(wx.Panel):
    ...
    def OnRun(self, event):
        self.rank=self.rank.GetValue()
        self.nrun=self.nrun.GetValue()
        self.update=self.update.GetString(self.update.GetCurrentSelection())
        self.objective=self.objective.GetString(self.objective.GetCurrentSelection())
        self.maxiter=self.maxiter.GetValue()
        try:
            self.W,self.H = nimfaRun(MainWindow.dataNew,self.rank,self.nrun,self.update,self.objective,self.maxiter)
    ...
    def PlotResult(self,event):
        try:
            NMFAuiWindow.plot(self, self.W, self.H)
        except AttributeError as error:
            print(Error)
            dlg=wx.MessageDialog(self, "No data loaded.", caption='Error!',
                style=wx.ICON_ERROR)
...
class scikitpanel(wx.Panel):
    ...
    def OnRun(self, event):
        self.component=self.component.GetValue()
        self.tolerance=float(self.tolerance.GetLineText(0))
        self.initialize=self.initialize.GetString(self.initialize.GetCurrentSelection())
        self.initialize=None if self.initialize=='None' else self.initialize  #Initialise default input is Empty-None
        self.betaloss=self.betaloss.GetString(self.betaloss.GetCurrentSelection())
        self.iteration=self.iteration.GetValue()
        try:
            self.W,self.H = scikitRun(MainWindow.dataNew,self.component,self.initialize,self.betaloss,self.tolerance,self.iteration)
